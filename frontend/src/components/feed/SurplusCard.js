...
import { MapPin, Package, BowlFood, Carrot, Truck, Handshake } from "@phosphor-icons/react";
...
export const SurplusCard = ({ post, index = 0 }) => {
  ...
  const dist = post.distance_m == null ? null : post.distance_m < 1000 ? `${post.distance_m} m` : `${(post.distance_m / 1000).toFixed(1)} km`;
  ...
  return (
    <motion.button
      ...
      className="press block w-full overflow-hidden rounded-3xl border border-line bg-white text-left shadow-warm"
    >
      <div className="relative h-44 w-full overflow-hidden bg-line">
        ...
        <div className="absolute inset-x-3 bottom-3 flex items-center gap-3 text-white">
          <span className="flex items-center gap-1 text-xs font-bold"><MapPin size={14} weight="fill" />{dist || "Lokasi di peta"}</span>
